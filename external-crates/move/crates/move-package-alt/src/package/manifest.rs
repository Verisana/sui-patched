...
mod tests {
    ...
    use crate::{
        flavor::{Vanilla, vanilla::default_environment},
        schema::PackageName,
    };
    ...
    fn load_manifest(contents: impl AsRef<[u8]>) -> ManifestResult<Manifest<Vanilla>> {
    ...
    fn dep_replacement_envs_are_declared() {
        ...
        assert!(
            manifest
                .dependencies()
                .contains(&String::from("mainnet"), &name)
        );
        let default_env = default_environment();
        assert!(!manifest.dependencies().contains(default_env.name(), &name));
    }
}
