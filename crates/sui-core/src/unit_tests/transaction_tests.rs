...
async fn setup_zklogin_network(
    ...
    let jwks = parse_jwks(DEFAULT_JWK_BYTES, &OIDCProvider::Twitch, false).unwrap();
    let epoch_store = authority_state.epoch_store_for_testing();
...
async fn zklogin_txn_fail_if_missing_jwk() {
    ...
    let jwks = parse_jwks(DEFAULT_JWK_BYTES, &OIDCProvider::Google, false).unwrap();
    let epoch_store = authority_state.epoch_store_for_testing();
    epoch_store.update_authenticator_state(&AuthenticatorStateUpdate {
        epoch: 0,
        round: 0,
    ...
    let jwks = parse_jwks(BAD_JWK_BYTES, &OIDCProvider::Twitch, false).unwrap();
    epoch_store.update_authenticator_state(&AuthenticatorStateUpdate {
        epoch: 0,
        round: 0,
        new_active_jwks: jwks
